...
(() => {
  ...
  function block(title, note2, ...children) {
    return el(
      "section",
      { class: "block" },
      el("h2", {}, title),
      note2 ? el("p", { class: "note" }, note2) : null,
  ...
  function pluginsAt(point) {
    return PLUGINS.filter((plugin) => plugin.point === point);
  }

  // src/model/policy.ts
  var API_VERSION = "descheduler/v1alpha2";
  ...
  function addProfile(doc, name) {
    ...
    const list = Array.isArray(doc["profiles"]) ? doc["profiles"] : [];
    list.push(profile);
    doc["profiles"] = list;
    return profile;
  }
  ...
  function isEnabled(profile, name, point) {
  ...
  function setEnabled(profile, name, point, on) {
    const set2 = pluginSet(profile, point, true);
    if (!set2) return;
    const enabled = names(set2, "enabled").filter((item) => item !== name);
    if (on) enabled.push(name);
    if (enabled.length) set2["enabled"] = enabled;
    else delete set2["enabled"];
    const disabled = names(set2, "disabled").filter((item) => item !== name);
    if (disabled.length) set2["disabled"] = disabled;
    else delete set2["disabled"];
    const plugins = profile["plugins"];
    if (isRecord(plugins) && !Object.keys(set2).length) delete plugins[point];
  }
  ...
  function ensureArgs(profile, name) {
  ...
  function removeArgs(profile, name) {
  ...
  function setPath(root, path, value) {
    ...
    if (value === void 0) delete here[last];
    ...
    for (let i = chain.length - 1; i > 0; i--) {
      ...
      const key = steps[i - 1];
      if (key !== void 0) delete parent[key];
    }
  }
  ...
  function problems(doc) {
    ...
    if (version && !version.startsWith("descheduler/")) {
      found2.push(`apiVersion is "${version}", which is not a descheduler policy version`);
    }
    const list = profiles(doc);
    if (!list.length) found2.push("the policy has no profiles, so nothing would run");
    for (const profile of list) {
      const where = String(profile["name"] ?? "(unnamed)");
      if (!profile["name"]) found2.push("a profile has no name");
      if (!enabledPlugins(profile).length) {
        found2.push(`profile "${where}" enables no plugins, so it would do nothing`);
      }
      for (const name of enabledPlugins(profile)) {
        const args = argsOf(profile, name);
        found2.push(...argProblems(where, name, args));
      }
      const evictor = argsOf(profile, DEFAULT_EVICTOR);
      const threshold = evictor["priorityThreshold"];
      if (isRecord(threshold) && threshold["value"] !== void 0 && threshold["name"] !== void 0) {
        found2.push(`profile "${where}": the evictor names both a priority value and a PriorityClass`);
      }
      found2.push(...namespaceProblems(where, DEFAULT_EVICTOR, evictor));
    }
    return found2;
  }
  function argProblems(where, name, args) {
    const found2 = [];
    ...
    for (const key of ["thresholds", "targetThresholds"]) {
      ...
      if (!isRecord(value)) {
        found2.push(`${at}: ${key} is not a mapping of resource to percentage`);
        continue;
      }
      for (const [resource, percentage] of Object.entries(value)) {
        const number = Number(percentage);
        if (!Number.isFinite(number) || number < 0 || number > 100) {
          found2.push(`${at}: ${key}.${resource} is ${String(percentage)}, and a percentage is 0 to 100`);
        }
      }
    }
    if (name === "LowNodeUtilization" || name === "HighNodeUtilization") {
      if (!isRecord(args["thresholds"]) || !Object.keys(args["thresholds"]).length) {
        found2.push(`${at}: needs thresholds, and has none`);
      }
    }
    if (name === "LowNodeUtilization") {
      ...
      if (!isRecord(high) || !Object.keys(high).length) {
        found2.push(`${at}: needs targetThresholds, and has none`);
        ...
        for (const [resource, value] of Object.entries(low)) {
          ...
          if (target !== void 0 && Number(value) > Number(target)) {
            found2.push(`${at}: ${resource} is under-used above ${String(value)}% but over-used above ${String(target)}%`);
          }
        }
      }
    }
    if (name === "RemovePodsViolatingNodeAffinity") {
      const types2 = args["nodeAffinityType"];
      if (!Array.isArray(types2) || !types2.length) {
        found2.push(`${at}: needs a nodeAffinityType, or it does nothing`);
      }
    }
    if (name === "RemovePodsHavingTooManyRestarts") {
      const threshold = Number(args["podRestartThreshold"] ?? 0);
      if (!Number.isFinite(threshold) || threshold < 1) {
        found2.push(`${at}: podRestartThreshold must be 1 or more`);
      }
    }
    if (name === "PodLifeTime") {
      const filters = ["maxPodLifeTimeSeconds", "states", "conditions", "exitCodes"];
      if (!filters.some((key) => args[key] !== void 0)) {
        found2.push(`${at}: needs at least one of an age, states, conditions or exit codes`);
      }
    }
    found2.push(...namespaceProblems(where, name, args));
    return found2;
  }
  function namespaceProblems(where, name, args) {
    const found2 = [];
    for (const key of ["namespaces", "evictableNamespaces"]) {
      ...
      if (include.length && exclude.length) {
        found2.push(`profile "${where}", ${name}: ${key} has both an include and an exclude list`);
      }
    }
    return found2;
  }
  ...
  function emptyArgs(doc) {
    ...
    for (const profile of profiles(doc)) {
      ...
      for (const config of list) {
        ...
        if (!isRecord(args) || !Object.keys(args).length) found2.add(`${where}|${String(config["name"] ?? "")}`);
      }
    }
    return found2;
  }
  function tidy(doc, keep) {
  ...
  function chipList(values, write2, options = {}) {
    const holder = el("div", { class: "chips" });
    const draw = () => {
      const parts = values.map(
        (value, index) => chip(value, () => {
      ...
      const add = () => {
        ...
        box.value = "";
        draw();
        holder.querySelector(".chip-input")?.focus();
      };
  ...
  function field(spec, target, changed) {
    const current2 = getPath(target.read(), spec.path);
    const write2 = (value) => {
    ...
    switch (spec.kind) {
      case "bool": {
        const box = el("input", { type: "checkbox", class: "switch", ...current2 === true ? { checked: "checked" } : {} });
        box.addEventListener("change", () => write2(box.checked ? true : void 0));
        const id = `field-${++uniqueId}`;
        box.id = id;
        return el(
          "div",
      ...
      case "number": {
        const input = el("input", {
          ...
          value: current2 === void 0 || current2 === null ? "" : String(current2)
        });
        input.addEventListener("change", () => {
          const text = input.value.trim();
          if (!text) return write2(void 0);
          const value = Number(text);
          return write2(Number.isFinite(value) ? value : void 0);
        });
      ...
      case "text": {
        ...
        input.addEventListener("change", () => write2(input.value.trim() || void 0));
      ...
      case "labels": {
        const input = el("input", {
          ...
          class: "input",
          ...spec.placeholder ? { placeholder: spec.placeholder } : {},
          value: formatLabels(current2)
        });
        input.addEventListener("change", () => {
          const labels = parseLabels(input.value);
          write2(Object.keys(labels).length ? labels : void 0);
          input.value = formatLabels(getPath(target.read(), spec.path));
        });
        return row(spec, input);
      }
      case "list": {
        const values = asArray(current2);
        const list = chipList(
          values,
          (next) => write2(next.length ? spec.numeric ? next.map(Number) : next : void 0),
          { suggest: spec.suggest, numeric: spec.numeric }
        );
        return row(spec, list);
      }
      case "choice": {
        const chosen = new Set(asArray(current2));
        ...
        for (const option of spec.options) {
          ...
          box.addEventListener("change", () => {
            ...
            write2(chosen.size ? [...chosen] : void 0);
          });
      ...
      case "select": {
        const picker2 = el("select", { class: "input" });
        picker2.append(el("option", { value: "" }, "(not set)"));
        for (const option of spec.options) {
          picker2.append(el("option", { value: option, ...current2 === option ? { selected: "selected" } : {} }, option));
        }
        picker2.addEventListener("change", () => write2(picker2.value || void 0));
        return row(spec, picker2);
      }
      case "namespaces": {
        ...
        const refresh = () => {
          warn.style.display = asArray(getPath(target.read(), `${spec.path}.include`)).length && asArray(getPath(target.read(), `${spec.path}.exclude`)).length ? "block" : "none";
        };
        both.append(
          el(
            "div",
            ...
            chipList(include, (next) => {
              setPath(target.write(), `${spec.path}.include`, next.length ? next : void 0);
              refresh();
              changed();
          ...
          el(
            ...
            chipList(exclude, (next) => {
              setPath(target.write(), `${spec.path}.exclude`, next.length ? next : void 0);
              refresh();
              changed();
            }, { placeholder: "namespace…" })
          )
      ...
      case "thresholds": {
        ...
        for (const resource of ["cpu", "memory", "pods"]) {
          ...
          input.addEventListener("change", () => {
            const text = input.value.trim();
            if (!text) {
              setPath(target.write(), `${spec.path}.${resource}`, void 0);
            } else {
              const number = Number(text);
              setPath(target.write(), `${spec.path}.${resource}`, Number.isFinite(number) ? number : void 0);
            }
            changed();
          });
          holder.append(
        ...
        const known = /* @__PURE__ */ new Set(["cpu", "memory", "pods"]);
        const extra = Object.keys(getPath(target.read(), spec.path) ?? {}).filter((key) => !known.has(key));
        return row(
          spec,
          holder,
          extra.length ? el("p", { class: "field-warn" }, `Also set, and left as they are: ${extra.join(", ")}.`) : null
        );
      }
    }
  }
  function fields(specs, target, changed) {
  ...
  function flags(tokens) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (equals >= 0) {
        found2.set(name.slice(0, equals), name.slice(equals + 1));
        continue;
      }
      const next = tokens[i + 1];
      if (next !== void 0 && !next.startsWith("-")) {
        found2.set(name, next);
        i++;
      } else {
        found2.set(name, "true");
      }
    }
    return found2;
  }
  function policySource(spec, namespace, path) {
    ...
    const file2 = path.slice(slash + 1);
    for (const container of containersOf(spec)) {
      for (const mount of container.volumeMounts ?? []) {
        ...
        const wanted = single ? mount.subPath ?? file2 : file2;
  ...
  function findInstalls(deployments, cronJobs) {
    ...
    return found2.sort((a, b) => (a.namespace + a.name).localeCompare(b.namespace + b.name));
  }
  ...
  var showFile = false;
  var creating = false;
  var keepEmpty = /* @__PURE__ */ new Set();
  start("page", async () => {
    found = await findDescheduler();
    file = await readPolicy(found.install);
    original = file.doc;
    keepEmpty = file.doc ? emptyArgs(file.doc) : /* @__PURE__ */ new Set();
  ...
  function drawSource() {
    ...
    byId("lead").textContent = install ? `What the descheduler in ${install.namespace}/${install.name} is told to do. It runs ${cadence(install)}, and reads this file ${install.mode === "cronjob" ? "at the start of every run" : "when its pod starts"}.` : "Nothing in this cluster runs the descheduler, so this is the policy a descheduler here would read.";
    const rows = [];
    if (file.configMap) {
      rows.push([
        "File",
  ...
  function pluginBlock(title, note2, point, profile) {
    const host = el("div", {});
    for (const spec of pluginsAt(point)) host.append(pluginCard(spec, point, profile));
    return block(title, note2, host);
  }
  function pluginCard(spec, point, profile) {
    const card = el("div", { class: "plugin" });
    const draw = () => {
      const on = isEnabled(profile, spec.name, point);
      const box = el("input", { type: "checkbox", class: "switch", ...on ? { checked: "checked" } : {} });
      box.addEventListener("change", () => {
  ...
  function drawFoot() {
    ...
    const yaml2 = dumpPolicy(written);
    ...
    replace(
      ...
      block(
        ...
        dirty ? "The app shows you this file and asks before anything is written. Comments and the original layout are not kept — the file below is exactly what would be applied." : "The form matches what is in the cluster.",
        wrong.length ? el(
          "div",
          { class: "problems" },
        ...
        buttons,
        showFile ? el("pre", { class: "yaml" }, yaml2) : null
      )
    );
  }
  function afterwards() {
  ...
  async function write(yaml2) {
    ...
    try {
      if (file.configMap) {
        await k8sdockside.patch({ kind: "configmaps", namespace, name, patch: { data: { [key]: yaml2 } } });
      } else {
      ...
      await k8sdockside.patch({
        ...
        patch: {
          spec: {
            template: {
              metadata: {
                annotations: { "kubectl.kubernetes.io/restartedAt": (/* @__PURE__ */ new Date()).toISOString() }
              }
            }
          }
        }
